...
public class TileCoprocCrypto extends TileCoprocBase {
		...
		public Object[] callMethod(ILuaContext ctx, int arg0, Object[] args) throws LuaException {
			...
			case 0:
				...
				try {
					...
					throw new LuaException("unknown algorithm: "+algorithm);
				} catch(NoSuchPaddingException e) {
					throw new LuaException("unknown padding: "+algorithm);
				}
				
				try {
					cipher.init(Cipher.ENCRYPT_MODE, key);
				} catch(InvalidKeyException e) {
					throw new LuaException("invalid key for algorithm: "+e.getMessage());
				}
				...
				try {
					...
					throw new LuaException("illegal block size: "+e.getMessage());
				} catch (BadPaddingException e) {
					throw new LuaException("illegal padding: "+e.getMessage());
				}
				...
				if(params != null)
					try {
						...
						e1.printStackTrace();
						throw new LuaException(e1.toString());
					}
				else
			...
			case 1:
				...
				if(input.indexOf(':') >= 0) {
					...
					i = input.indexOf(':');
					if(i < 0)
						throw new LuaException("malformed input");
					...
					try {
						...
						throw new LuaException("No such algorithm "+paramAlgorithm);
					} catch (IOException e) {
						throw new LuaException("failed reading parameters: "+e.toString());
					}
				} else {
					params = null;
				...
				try {
					...
					throw new LuaException("invalid algorithm parameter: "+e.getMessage());
				} catch(InvalidKeyException e) {
					throw new LuaException("invalid key for algorithm: "+e.getMessage());
				}
				...
				try {
					...
					throw new LuaException("padding error: "+e.getMessage());
				} catch(IllegalBlockSizeException e) {
					throw new LuaException("illegal block size: "+e.getMessage());
				}
				
				return new Object[] {new String(plaintext, StandardCharsets.UTF_8)};
	...
	@Override
	public Object[] callMethod(IComputerAccess arg0, ILuaContext ctx, int arg1, Object[] args) throws LuaException {
		switch(arg1) {
		case 0:
			...
			if(keySize > MAX_ASYMMETRIC_KEY_SIZE)
				throw new LuaException("max key size is "+MAX_ASYMMETRIC_KEY_SIZE);
			...
			try {
				...
				throw new LuaException("invalid key size: "+keySize);
			}
		...
		case 1:
			...
			try {
				...
				throw new LuaException("unknown padding type");
			}
		...
		case 3:
			...
			int i = str.indexOf(':');
			if(i < 0)
				throw new LuaException("invalid encoded key");
			...
			if(fmt.equals("RAW")) {
				...
				try {
					...
					throw new LuaException("invalid encoded key: "+e.getMessage());
				}
				...
				throw new LuaException("unknown key format "+fmt);
			}
		...
		case 4:
			...
			algorithm = (String)args[0];
			byte[] input = ((String)args[1]).getBytes(StandardCharsets.UTF_8);
