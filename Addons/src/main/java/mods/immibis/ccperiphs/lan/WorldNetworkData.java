...
public class WorldNetworkData extends WorldSavedData {
	...
	public static WorldNetworkData getForWorld(World worldObj) {
		...
		WorldNetworkData rv = (WorldNetworkData)worldObj.getPerWorldStorage().getOrLoadData(WorldNetworkData.class, DATA_NAME);
		if(rv == null) {
			rv = new WorldNetworkData(DATA_NAME);
			worldObj.getPerWorldStorage().setData(DATA_NAME, rv);
		}
		return rv;
	}
	...
	public void readFromNBT(NBTTagCompound tag) {
		...
		NBTTagList list = tag.getTagList("nets", NBT.TAG_COMPOUND);
		for(int k = 0; k < list.tagCount(); k++) {
			NBTTagCompound netTag = list.getCompoundTagAt(k);
			
			CableNet net = new CableNet(netTag.getByte("type"));
			
			NBTTagList cablesTag = netTag.getTagList("cables", NBT.TAG_COMPOUND);
			for(int i = 0; i < cablesTag.tagCount(); i++) {
				NBTTagCompound cableTag = cablesTag.getCompoundTagAt(i);
			...
			NBTTagList nicsTag = netTag.getTagList("nics", NBT.TAG_COMPOUND);
			for(int i = 0; i < nicsTag.tagCount(); i++) {
				NBTTagCompound nicTag = nicsTag.getCompoundTagAt(i);
