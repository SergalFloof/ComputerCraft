...
public class TileRFIDWriter extends TilePeriphs implements IPeripheral, IInventory, IIPPeripheralTile {
	...
	public void onDataPacket(SPacketUpdateTileEntity p) {
		int i = p.getNbtCompound().getInteger("i");
		facing = (byte)i;
		state = (byte)(i >> 8);
	...
	private boolean canRun() {
		if(contents == null || contents.getItem() != ImmibisPeripherals.itemRFID)
			return false;
	...
	private synchronized boolean isHoldingCard() {
		return contents != null && contents.getItem() == ImmibisPeripherals.itemRFID;
	}
	...
	public synchronized Object[] callMethod(IComputerAccess computer, ILuaContext ctx, int method, Object[] arguments) throws LuaException {
		...
		case 0: // encode
			if(progress > 0)
				throw new LuaException("Already encoding");
			if(contents == null)
				throw new LuaException("No card inserted");
			if(contents.getItem() != ImmibisPeripherals.itemRFID)
				throw new LuaException("Foreign object blocking slot");
			if(contents.stackTagCompound != null && contents.stackTagCompound.hasKey("data"))
				throw new LuaException("Card already encoded");
			if(arguments.length != 2 && arguments.length != 3)
				throw new LuaException("Expected 2 arguments");
			for(int k = 0; k < arguments.length; k++)
				if(!(arguments[k] instanceof String))
					throw new LuaException("Expected string as argument "+k);
	...
	public boolean isItemValidForSlot(int i, ItemStack itemstack) {
		//return itemstack != null && itemstack.getItem() == ImmibisPeripherals.itemRFID;
		return true;
	}
