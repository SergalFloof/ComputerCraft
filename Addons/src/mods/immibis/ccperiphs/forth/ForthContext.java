...
public class ForthContext {
	...
	private static int[] shortQueueToIntArray(Queue<Short> q) {
		int[] rv = new int[q.size()];
		int k = 0;
		for(short s : q)
			rv[k++] = s;
		return rv;
	...
	public NBTTagCompound save() {
		...
		tag.setIntArray("immed", shortQueueToIntArray(immed));
		
		int[] ar = new int[rstack_pos+1];
		System.arraycopy(rstack, 0, ar, 0, rstack_pos+1);
		tag.setIntArray("rstack", ar);
		
		ar = new int[stack.size()];
		for(int k = 0; k < stack.size(); k++)
			ar[k] = (Integer)stack.get(k);
		tag.setIntArray("stack", ar);
	...
	public void load(NBTTagCompound tag) {
		...
		immed.clear();
		for(int i : tag.getIntArray("immed"))
			immed.add((short)i);
		
		int[] ar = tag.getIntArray("rstack");
		rstack_pos = Math.min(ar.length - 1, rstack.length - 1);
		for(int k = 0; k <= rstack_pos; k++)
			rstack[k] = ar[k];
		
		stack.clear();
		for(int a : tag.getIntArray("stack"))
			stack.push(a);
		
		forth_dict.clear();
		NBTTagList list = tag.getTagList("forth_dict", NBT.TAG_COMPOUND);
		for(int k = 0; k < list.tagCount(); k++) {
			NBTTagCompound e = list.getCompoundTagAt(k);
