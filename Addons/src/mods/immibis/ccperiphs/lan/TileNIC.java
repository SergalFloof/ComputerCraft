 ...
 */
public class TileNIC extends TilePeriphs implements IPeripheral, IIPPeripheralTile {
	...
	public Object[] callMethod(IComputerAccess computer, ILuaContext ctx, int method, Object[] arguments) throws LuaException {
		...
		case 0: // send
			if(arguments.length < 1)
				throw new LuaException("Need one argument");
			if(!(arguments[0] instanceof String))
				throw new LuaException("Argument must be string");
			
			synchronized(this) {
				ch = defaultSendChannel;
			}
			if(approxSendQueueLength.get() >= MAX_SEND_QUEUE_LENGTH)
				throw new LuaException("Buffer full");
		...
		case 1: // sendChannel
			if(arguments.length < 2)
				throw new LuaException("Need two arguments");
			if(!(arguments[0] instanceof Double))
				throw new LuaException("First argument must be number");
			if(!(arguments[1] instanceof String))
				throw new LuaException("Second argument must be string");
			
			ch = (int)(double)(Double)arguments[0];
			if(ch > MAX_CHANNEL)
				throw new LuaException("Channel out of range (must be "+MAX_CHANNEL+" or lower)");
		...
		case 2: // setDefaultChannel
			if(arguments.length < 1 || !(arguments[0] instanceof Double))
				throw new LuaException("Argument must be number");
		...
		case 4: // setPromiscuous
			if(arguments.length < 1 || !(arguments[0] instanceof Boolean))
				throw new LuaException("Argument must be boolean");
			synchronized(this) {
		...
		case 6: // sendFrom
			if(!ImmibisPeripherals.enableSendFrom)
				throw new LuaException("sendFrom is disabled on this server");
			
			if(arguments.length < 3)
				throw new LuaException("Need three arguments");
			if(!(arguments[0] instanceof Double) || !(arguments[1] instanceof Double))
				throw new LuaException("First and second arguments must be numbers");
			if(!(arguments[2] instanceof String))
				throw new LuaException("Third argument must be string");
		...
		case 7: // setListening
			...
			if(!(arguments[1] instanceof Boolean))
				throw new LuaException("Second argument must be boolean");
			
			ch = (int)(double)(Double)arguments[0];
			if(ch < 1 || ch > MAX_CHANNEL)
				throw new LuaException("Channel out of range (must be between 1 and "+MAX_CHANNEL+" inclusive)");
	...
	public synchronized void onDataPacket(S35PacketUpdateTileEntity packet) {
		facing = (byte)(packet.func_148853_f() & 7);
		isConnectedToComputer = (packet.func_148853_f() & 8) != 0;
		worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
	}
