...
public class NetworkingManager implements INetworkingManager, ChannelInboundHandler {
	...
	private static Packet createCustomPayloadPacket(String channel, byte[] data, boolean isFromServer) {
		if(isFromServer)
			...
			return new CPacketCustomPayload();
	}
	...
	public void sendToServer(IPacket packet) {
		...
		send250(packet, Minecraft.getMinecraft().player.sendQueue.getNetworkManager(), false);
	}
	...
	public void sendToServer(ISimplePacket packet) {
		NetworkManager netManager = Minecraft.getMinecraft().player.sendQueue.getNetworkManager();
	...
	static {
		try {
			for(Field f : EnumConnectionState.class.getDeclaredFields()) {
				...
				if(f.getType() == Map.class && (f.getModifiers() & Modifier.STATIC) != 0 && ((Map)f.get(null)).containsKey(SPacketCustomPayload.class)) {
	...
	public void channelRead(ChannelHandlerContext arg0, Object arg1) throws Exception {
		if(arg1 instanceof CPacketCustomPayload) {
			CPacketCustomPayload packet = ((CPacketCustomPayload)arg1);
			if(channels.containsKey(packet.getChannelName())) {
				onReceivePacket(packet.getChannelName(), new DataInputStream(new ByteArrayInputStream(packet.getBufferData())), false, arg0);
				return;
			}
		}
		if(arg1 instanceof SPacketCustomPayload) {
			SPacketCustomPayload packet = ((SPacketCustomPayload)arg1);
			if(channels.containsKey(packet.getChannelName())) {
				onReceivePacket(packet.getChannelName(), new DataInputStream(new ByteArrayInputStream(packet.func_149168_d())), true, arg0);
				return;
			}
		}
