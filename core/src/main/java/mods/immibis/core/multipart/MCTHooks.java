...
public class MCTHooks {
	...
	public static boolean client_onBlockBreak(int x, int y, int z) {
		...
		BlockEvent.BreakEvent evt = new BlockEvent.BreakEvent(x, y, z, ply.world, ply.world.getBlock(x, y, z), ply.world.getBlockState(pos), ply);
	...
	public static void client_onBlockClicked() {
		...
		MultipartSystem.instance.clearBreakingPart(Minecraft.getMinecraft().player);
	...
	public static void server_removeBlockStart(ItemInWorldManager m, int x, int y, int z) {
		...
		lastRemovedBlockInfo.set("a "+Block.REGISTRY.getNameForObject(m.theWorld.getBlock(x, y, z))+" with tile entity "+(te == null ? "null" : te.getClass().getName()));
