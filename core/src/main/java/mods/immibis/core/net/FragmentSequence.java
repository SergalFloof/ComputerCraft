	...
	private static void kickPlayer(FragmentSequence sequence, final EntityPlayer ply,  String reason_) {
		...
		if(ply == null) {
			MainThreadTaskQueue.enqueue(new Runnable() {
				...
				public void run() {
					EntityClientPlayerMP ply = Minecraft.getMinecraft().player;
					if(ply != null && ply.sendQueue != null)
						ply.sendQueue.handleDisconnect(new SPacketDisconnect(new TextComponentString("Client: " + reason)));
				}
			}, Side.CLIENT);
		}
	}
