        ...
        ) )
        {
            switch( getPeripheralType() )
            {
                case WiredModem:
                {
                    ...
                    ((BlockGeneric)getBlockType()).dropAllItems( world, getPos(), 0, false, false );
                ...
                case WiredModemWithCable:
                {
                    ...
                    ((BlockGeneric)getBlockType()).dropItem( world, getPos(), PeripheralItemFactory.create( PeripheralType.WiredModem, getLabel(), 1 ) );
                    setLabel( null );
    	...
    	if( !world.isRemote )
    	{
    		...
    		else
    		{
    			...
    			for( EnumFacing dir : EnumFacing.values() )
    			{
					...
					if( offset.getY() >= 0 && offset.getY() < world.getHeight() && BlockCable.isCable( world, offset ) )
					{
    ...
    private IPeripheral getConnectedPeripheral()
    {
    	if( m_peripheralAccessAllowed )
    	{
			if( getPeripheralType() == PeripheralType.WiredModemWithCable )
			{
				...
				return PeripheralUtil.getPeripheral( world, neighbour, facing.getOpposite() );
			}
		}
		return null;
	}
